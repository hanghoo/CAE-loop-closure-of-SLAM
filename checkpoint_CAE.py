...
optimizer = tf.train.RMSPropOptimizer(learning_rate).minimize(mse_error,global_step=global_epoch)
...
with tf.Session() as sess:
    ...
    CHECKPOINT_DIR="/home/plz/AWS_EC2_codes/plz-cae"
    if not RESTART:
        checkpoint=tf.train.latest_checkpoint(CHECKPOINT_DIR)
        if checkpoint:
            saver.restore(sess,checkpoint)
            print("has restored from last checkpoint")
        else:
            print("unable to restore from last checkpoint")
            
    old_epoch=sess.run(global_epoch)
  
    # train cycle

    total_batch = int(files_num / train_batch_size)
    for epoch in range(old_epoch,MAX_epoches):
    ...
    image_set_DIR="/home/plz/slam/Images_12/"
    
    filenames = os.listdir(image_set_DIR)

    features_list = []
    for image in filenames:
        im_test = Image.open(image_set_DIR+image)
